...
/// with it. Echoed by the client: the selection endpoint keeps no record of
/// its answers, and only the client knows whether the answer it got is the
/// one it applied.
#[derive(Debug, Clone, PartialEq, Eq, Serialize)]
pub struct SessionVerdictSelection {
    /// The answer as applied: `full`, `subset`, `empty` or `refused`.
    pub answer: String,
    /// The engine's own word, forwarded verbatim — or the client's, where it
    /// degraded the answer itself.
    pub reason: String,
    /// How many tests the selection left the run to run.
    pub kept_count: u32,
    /// Why a served selection could not be applied and the whole suite ran
    /// instead. `None` when it was applied.
